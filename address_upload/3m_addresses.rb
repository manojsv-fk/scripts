...
class BatchUpdate
  ...
  ROW_HEADING = [:address_id, :location_id, :ship_to, :stop_name, :address_line_1, :address_line_2, :city, :state, :zip, :country,
    :load_unload_time, :latitude, :longitude, :geofence_radius, :polygon_coordinates, :tags]
  ...
  def map_csv_headers
    ...
    csv_headers.each do |col_name|
      ...
      if col.include? "lcs_fclt_code"
        self.headers[:location_id] = col_name
        next
      end
      
      if col.include? "lcs_fclt_name"
        self.headers[:stop_name] = col_name
        next
      end
      
      if col.include? "addr" and col.include? "ln" and col.include? "1"
        self.headers[:address_line_1] = col_name
        next
      end
      
      if col.include? "addr" and col.include? "ln" and col.include? "2"
      ...
      if col.include? "ste_prov_code"
        self.headers[:state] = col_name
        next
      end
      
      if col.include? "pstl_code"
  ...
  def batch_create(payloads, batch_num)
    ...
    begin
      ...
      if response_hash["statusCode"] == 200
          ...
          puts "Error occured in batch #{batch_num}"
          puts response_hash
...
/Users/manojsv/Desktop/trac6267.csv
