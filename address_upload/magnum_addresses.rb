...
class BatchUpdate
  ...
  def map_csv_headers
    ...
    csv_headers.each do |col_name|
      ...
      if col.include? "address" and col.include? "1"
        self.headers[:address_line_1] = col_name
        next
      end
      
      if col.include? "address" and col.include? "2"
      ...
      if col.include? "open" and col.include? "time"
      ...
      if col.include? "close" and col.include? "time"
  ...
  def create_row_hash(row)
    ...
    row.each do |key, value|
      ...
      if key.include? 'OPEN' and key.include? 'TIME'
        row_hash[:tues_open] = value
        row_hash[:wed_open] = value
        row_hash[:thurs_open] = value
        row_hash[:fri_open] = value
        row_hash[:sat_open] = value
        row_hash[:sun_open] = value
      elsif key.include? 'CLOSE' and key.include? 'TIME'
        row_hash[:tues_close] = value
        row_hash[:wed_close] = value
        row_hash[:thurs_close] = value
        row_hash[:fri_close] = value
        row_hash[:sat_close] = value
        row_hash[:sun_close] = value
      end  
    end
  ...
  def get_cut_off_times(address_hash)
    ...
    0.upto(6) do |day_num|
      ...
      start_cut_off_time = start_cut_off_time[0...5] if start_cut_off_time.size > 5
      end_cut_off_time = end_cut_off_time[0...5] if end_cut_off_time.size > 5
...
/Users/manojsv/Desktop/magnum_addresses.csv
