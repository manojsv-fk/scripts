...
class CustomerUploader
  ...
  HEADING_HASH = {customer_id: "ID's", customer_name: 'Customer Name', priority: 'Priority'}
  ...
  def get_row_hash(row)
    ...
    row.each do |heading , value|
      ...
      row_heading = :customer_name if (row_heading.nil? && heading.present? && heading.include?('Customer') && heading.include?('Name'))
  ...
  def populate_nil_values(row_hash)
   ...
   def create_customer(customer_hash, row_number)
    ...
    @high_priority << customer_hash[:customer_name] if customer_hash[:priority].downcase == 'high'
  ...
  def upload_customer(customer_name, index)
    begin
      ...
      puts "Customer to be uploaded #{index}"
      puts params
  ...
  def get_request_payload_json(customer_hash)
    ...
    priority = customer_hash[:priority].present? ? customer_hash[:priority].downcase : 'normal'
    priority = 'high' if @high_priority.include?(customer_hash[:name])
...
company = Company.find_by_permalink('exide-technologies')
customer_uploader = CustomerUploader.new( ["/Users/manojsv/Desktop/3m_customers.csv", company.id, "/Users/manojsv/Desktop/output.csv"])
customer_uploader = CustomerUploader.new( ["/home/ec2-user/manojsv/campbell_customers.csv", company.id, "/home/ec2-user/manojsv/campbell_customers_output.csv"])
