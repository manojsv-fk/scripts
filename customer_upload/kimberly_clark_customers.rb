...
class CustomerUploader
  ...
  def populate_nil_values(row_hash)
   ...
   def create_customer(customer_hash, row_number)
    ...
    customer = {}
    customer[:name] = customer_hash[:customer_name]
    customer[:identifiers] = get_comma_sepereated_ids(customer_hash[:customer_id])
    customer[:priority] = customer_hash[:priority]
    @cust_array << customer
    return
  end

  def upload_customer(customer_hash)
    begin
      identifiers = customer_hash[:identifiers]
      customer_hash = get_request_payload_json(customer_hash)
...
customer_uploader = CustomerUploader.new( ["/Users/manojsv/Desktop/trac6172.xlsx", company.id, "/Users/manojsv/Desktop/output.csv"])
customer_uploader = CustomerUploader.new( ["/home/ec2-user/manojsv/trac6172.xlsx", company.id, "/home/ec2-user/manojsv/trac6172_customers_output.csv"])
